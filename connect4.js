...
function findSpotForCol(x) {
  let firstEmptyIdx = board.indexOf(board.find((value) => value[x] !== null));
  if (firstEmptyIdx === -1) {
    firstEmptyIdx = HEIGHT;
  } else if (firstEmptyIdx === 0) {
    return null;
  }
  board[firstEmptyIdx - 1][x] = currPlayer;
  console.log(board);
...
function placeInTable(y, x) {
  const selectedPiece = document.getElementById(`${y}-${x}`);
  const newPiece = document.createElement("div");
  newPiece.classList.add("piece");
  const color = currPlayer === 1 ? "blue" : "red";
...
function checkFilled() {
  return board.every((value) => value.every((value) => value !== null));
}
